...
SELECT P.title AS 'Заголовок', P.full_text AS 'Текст публикации', P.user_id AS 'ID автора', U.login AS 'Автор'
...
SELECT P.id AS 'ID публикации', P.title AS 'Заголовок', P.full_text AS 'Текст публикации', U.id AS 'ID автора'
FROM posts P, users U
WHERE U.login = "Mikle" AND P.user_id = U.id;
